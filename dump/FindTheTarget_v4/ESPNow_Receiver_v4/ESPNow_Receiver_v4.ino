/* =====================================================================
 * ESPNow_Receiver_v4  --  Base
-station bridge
 * ---------------------------------------------------------------------
 * WHAT THIS BOARD DOES
 *   Sits on the base-station computer's USB port, receives SensorPackets
 *   from ESPNow_Sender_v4 over ESP-NOW, and forwards each packet to the
 ...
 *   it prints into PEER_MAC in ESPNow_Sender_v4.ino.
 *
...
void printJson(const SensorPacket &p, int8_t rssi) {
  ...
  Serial.print(F(",\"mag_ok\":"));       Serial.print((p.flags & FLAG_MAG_OK)   ? 1 : 0);
  ...
  Serial.print(F(",\"heading_deg\":"));  Serial.print(p.headingDeg, 1);
...
void setup() {
  ...
  Serial.println();
  Serial.println(F("# ==================================="));
  Serial.println(F("#  ESPNow_Receiver_v4  (base station)"));
  Serial.println(F("# ==================================="));
  ...
  Serial.println(F("# Paste that MAC into PEER_MAC in ESPNow_Sender_v4.ino"));
