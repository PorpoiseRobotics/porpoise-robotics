...
def lesson4(deck):
    ...
    deck.code(
        ...
        ["// STANDBY is not an animation. It is ONE PICTURE that changes",
         ...
         "case STANDBY:",
         "  if (standby_dirty) set_standby_lighting();",
         "  break;"],
    ...
    deck.code(
        ...
        ["void update_lighting(unsigned long now, uint8_t dpad) {",
         ...
         "    if (now - kitt_time < KITT_SPEED_MS) return;",
         "    kitt_time = now;",
...
def lesson5(deck):
    ...
    deck.code(
        ...
        ["uint16_t ina219_read16(uint8_t reg) {",
         ...
         "  int16_t raw = (int16_t)ina219_read16(REG_SHUNT);   // SIGNED",
         "  return (raw * 0.01f) / (SHUNT_RESISTOR_OHMS * 1000.0f);",
         "}",
         "",
         "float readBusVolts() {",
         "  uint16_t value = ina219_read16(REG_BUS);",
         "  return (float)((value >> 3) * 4) * 0.001f;   // top 13 bits, 4 mV/count",
         "}"],
        filename="Sensors.ino  /  a5b_ina219_current.ino",
